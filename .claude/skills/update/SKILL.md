---
name: update
description: Update old notes with new connections. The backward pass that /connect doesn't do. Revisit existing notes that predate newer related content, add connections, sharpen claims, consider splits. Triggers on "/update", "/update [note]", "update old notes", "backward connections", "revisit notes".
user-invocable: true
...
- If target contains `--handoff`: output ORCHESTRATE HANDOFF block at end
...
10. If `--handoff` in target: output ORCHESTRATE HANDOFF block
...
External loop mode for /orchestrate:
- Execute full workflow as normal
- At the end, output structured ORCHESTRATE HANDOFF block
...
When invoked with `--handoff`, output this structured format at the END of the session. This enables external loops (/orchestrate) to parse results and update the task queue.
...
```
=== ORCHESTRATE HANDOFF: {vocabulary.reweave} ===
Target: [[note name]]
...
### Task File Update (when invoked via orchestrate loop)

When running in handoff mode via /orchestrate, the prompt includes the task file path. After completing the workflow, update the `## {vocabulary.reweave}` section of that task file with:
...
When running interactively (NOT via /orchestrate), YOU must advance the phase in the queue. /orchestrate handles this automatically, but interactive sessions do not.
