...
```
=== ORCHESTRATE HANDOFF: {vocabulary.reflect} ===
Target: [[note name]]
...
When running in handoff mode via /orchestrate, the prompt includes the task file path. After completing the workflow, update the `## {vocabulary.reflect}` section of that task file with:
