...
- If target contains `--handoff`: output ORCHESTRATE HANDOFF block + task entries at end
...
7. If `--handoff` in target: create per-claim task files, update queue, output ORCHESTRATE HANDOFF block
...
- The chain: research query -> inbox file -> /{vocabulary.extract} -> {vocabulary.notes}
