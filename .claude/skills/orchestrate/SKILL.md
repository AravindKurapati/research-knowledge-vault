---
name: orchestrate
description: Queue processing with fresh context per phase. Processes N tasks from the queue, spawning isolated subagents to prevent context contamination. Supports serial, parallel, batch filter, and dry run modes. Triggers on "/orchestrate", "/orchestrate N", "process queue", "run pipeline tasks".
version: "1.0"
...
- --handoff: output structured ORCHESTRATE HANDOFF block at end (for pipeline chaining)
...
- Violates the orchestrate pattern (one phase per context window)
...
| reflect |  /connect | Find connections, update {DOMAIN:topic map}s |
| reweave |  /update | Update older {DOMAIN:note_plural} with new connections |
...
- Instruction to output ORCHESTRATE HANDOFF block
...
(1 entry per claim with current_phase/completed_phases), output ORCHESTRATE HANDOFF.
...
1. **Look for ORCHESTRATE HANDOFF block** — search for `=== ORCHESTRATE HANDOFF` and `=== END HANDOFF ===` markers
...
Check backward link gaps. Output ORCHESTRATE HANDOFF block when done.",
...
Use serial mode for per-phase filtering: /orchestrate N --type reflect
```
...
```
Orchestrate Lead (you) — orchestration only
|
...
**Why two phases?** Workers have sibling awareness (claim titles in spawn prompt) and link proactively during connect/update. But timing means some sibling notes may not exist yet during a worker's reflect phase. Phase B runs a single cross-connect pass after all notes exist.
...
Next steps:
  {if more pending tasks}: Run /orchestrate {remaining} to continue
...
```
=== ORCHESTRATE HANDOFF: orchestration ===
...
**Subagent crash mid-phase:** The queue still shows `current_phase` at the failed phase. The task file confirms the corresponding section is empty. Re-running `/orchestrate` picks it up automatically — the task is still pending at that phase.
...
Every subagent SHOULD return a ORCHESTRATE HANDOFF block. If missing: log warning, mark task done, continue.
