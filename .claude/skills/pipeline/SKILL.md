---
name: pipeline
description: End-to-end source processing -- seed, extract, process all claims through connect/update/verify, archive. The full pipeline in one command. Triggers on "/pipeline", "/pipeline [file]", "process this end to end", "full pipeline".
version: "1.0"
...
- `--handoff`: output ORCHESTRATE HANDOFF block at end (for chaining)
...
The pipeline chains four phases. Each phase uses skill invocation or /orchestrate for subagent-based processing. State lives in the queue file — the pipeline is stateless orchestration on top of stateful queue entries.
...
Phase 1: /seed — create extract task, move source to archive
    |
    v
Phase 2: /extract (via /orchestrate) — extract claims from source
    |
    v
Phase 3: /orchestrate (all claims) — create -> reflect -> reweave -> verify
    |
    v
...
The pipeline is the convenience wrapper. /orchestrate is the engine. /seed is the entry point.
...
Process the extract task via /orchestrate. This spawns a subagent that runs /extract, extracting claims from the source and creating task entries in the queue.

**How to invoke:**

```
/orchestrate 1 --batch {batch_id} --type extract
```

Or via Task tool:
```
Task(
  prompt = "Run /orchestrate 1 --batch {batch_id} --type extract",
...
```
/orchestrate {remaining_count} --batch {batch_id}
```

Or via Task tool:
```
Task(
  prompt = "Run /orchestrate {remaining_count} --batch {batch_id}",
...
Each phase runs in an isolated subagent with fresh context. /orchestrate handles all the orchestration: subagent spawning, handoff parsing, queue advancement, learnings capture.

**Progress reporting:**

The /orchestrate invocation reports progress per task. The pipeline relays this:
```
...
**For large batches (20+ claims):** /orchestrate handles context isolation automatically via subagents. The pipeline does NOT need to chunk — /orchestrate processes N tasks sequentially with fresh context per phase.

---

## Phase 4: Verify Completion

After /orchestrate finishes, verify all tasks for this batch are done.
...
- Suggest: "Run `/orchestrate --batch {batch_id}` to continue from where it stopped"
...
3. Suggest remediation: "Run `/orchestrate --batch {batch_id}` to continue from where it stopped"
...
- /orchestrate picks up from the last completed phase (queue is the source of truth)
...
**Processing failure:** If /orchestrate fails mid-batch, the queue preserves state. Individual claims resume from their failed phase on next /orchestrate invocation.
...
| After seed, before reduce | /orchestrate 1 --batch {id} --type extract |
| After reduce, during claims | /orchestrate --batch {id} (picks up from failed phase) |
...
- Process claims inline instead of via /orchestrate subagents
